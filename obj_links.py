        ...
        with open(file_name+".txt","r") as f:
            for line in f:
                ...
                if len(seps) == 5:
                    ...
                    new_dict = dict([('class_id', int(seps[0])), ('obj_id', int(seps[1])), ('centre_x', float(seps[2])), ('centre_y', float(seps[3])), ('bbox_width', float(seps[4])), ('bbox_height', float(seps[5]))])
                else:
            ...
            for cur_obj in sorted(object_lists, key = lambda k: k['class_id']):
                ...
                for prv_obj in sorted(prev_object_lists, key = lambda k:k['class_id']):
                    if cur_obj['class_id'] == prv_obj['class_id']:
                        ...
                        #ncc_cost = measure.compare_ssim(cur_obj_patch, resized_prv_obj_patch)
